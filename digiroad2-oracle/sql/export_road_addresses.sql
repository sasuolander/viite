...
DECLARE
    ...
    TYPE coordArrayType IS TABLE OF INTEGER INDEX BY VARCHAR2 (64);
    coordinate_array         coordArrayType;
    ...
    CURSOR links            (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT l.*
                                                                       FROM LINK l
                                                                       WHERE l.ID IN
                                                                             (
                                                                                 SELECT ll.LINK_ID
                                                                                 ...
                                                                                 WHERE ll.VALID_TO IS NULL
                                                                                   AND ll.ROADWAY_NUMBER IN
                                                                                       (
                                                                                           SELECT rw.ROADWAY_NUMBER
                                                                                           FROM ROADWAY rw
                                                                                           WHERE rw.VALID_TO IS NULL
                                                                                             AND rw.END_DATE IS NULL
                                                                                             AND rw.ROAD_NUMBER =
                                                                                                 ROAD
                                                                                             AND rw.ROAD_PART_NUMBER =
                                                                                                 ROAD_PART));
    CURSOR roads            (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT rw.*
                                                                       FROM ROADWAY rw
                                                                       WHERE rw.VALID_TO IS NULL
                                                                         AND rw.END_DATE IS NULL
                                                                         AND rw.ROAD_NUMBER =
                                                                         ...
                                                                         AND rw.ROAD_PART_NUMBER =
                                                                             ROAD_PART;
    CURSOR linear_locations (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT ll.*
                                                                       FROM LINEAR_LOCATION ll
                                                                       WHERE ll.VALID_TO IS NULL
                                                                         AND ll.ROADWAY_NUMBER IN
                                                                             (
                                                                                 ...
                                                                                 WHERE rw.VALID_TO IS NULL
                                                                                   ...
                                                                                   AND rw.ROAD_PART_NUMBER =
                                                                                       ROAD_PART);
    CURSOR rw_points        (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT rp.*
                                                                       FROM ROADWAY_POINT rp
                                                                       WHERE rp.ROADWAY_NUMBER IN
                                                                             (
                                                                                 SELECT rw.ROADWAY_NUMBER
                                                                                 FROM ROADWAY rw
                                                                                 WHERE rw.VALID_TO IS NULL
                                                                                   ...
                                                                                   AND rw.ROAD_PART_NUMBER =
                                                                                       ROAD_PART);
    CURSOR cal_points       (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT cp.*
                                                                       FROM CALIBRATION_POINT cp
                                                                       WHERE cp.VALID_TO IS NULL
                                                                         AND cp.ROADWAY_POINT_ID IN
                                                                             (
                                                                                 SELECT rp.ID
                                                                                 FROM ROADWAY_POINT rp
                                                                                 WHERE rp.ROADWAY_NUMBER IN
                                                                                       (
                                                                                           ...
                                                                                           WHERE rw.VALID_TO IS NULL
                                                                                             ...
                                                                                             AND rw.ROAD_PART_NUMBER =
                                                                                                 ROAD_PART));
    CURSOR nodes            (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT n.*
                                                                       FROM NODE n
                                                                       WHERE n.VALID_TO IS NULL
                                                                         AND n.END_DATE IS NULL
                                                                         AND n.NODE_NUMBER IN
                                                                             (
                                                                                 SELECT np.NODE_NUMBER
                                                                                 FROM NODE_POINT np
                                                                                 WHERE np.VALID_TO IS NULL
                                                                                   AND np.ROADWAY_POINT_ID IN
                                                                                       (
                                                                                           SELECT rp.ID
                                                                                           FROM ROADWAY_POINT rp
                                                                                           WHERE rp.ROADWAY_NUMBER IN
                                                                                 ...
                                                                                 UNION
                                                                                 SELECT j.NODE_NUMBER
                                                                                 FROM JUNCTION j
                                                                                          JOIN JUNCTION_POINT jp
                                                                                               ON (j.ID =
                                                                                                   jp.JUNCTION_ID)
                                                                                 WHERE j.VALID_TO IS NULL
                                                                                   AND j.END_DATE IS NULL
                                                                                   AND jp.VALID_TO IS NULL
                                                                                   AND jp.ROADWAY_POINT_ID IN
                                                                                       (
                                                                                           SELECT rp.ID
                                                                                           FROM ROADWAY_POINT rp
                                                                                           WHERE rp.ROADWAY_NUMBER IN
                                                                                                 (
                                                                                                     ...
                                                                                                     WHERE rw.VALID_TO IS NULL
                                                                                                       ...
                                                                                                       AND rw.ROAD_PART_NUMBER =
                                                                                                           ROAD_PART)));
    CURSOR node_points      (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT *
                                                                       FROM NODE_POINT np
                                                                       WHERE np.VALID_TO IS NULL
                                                                         AND np.ROADWAY_POINT_ID IN
                                                                             (
                                                                                 ...
                                                                                 WHERE rp.ROADWAY_NUMBER IN
                                                                                       (
                                                                                           ...
                                                                                           WHERE rw.VALID_TO IS NULL
                                                                                             ...
                                                                                             AND rw.ROAD_PART_NUMBER =
                                                                                                 ROAD_PART));
    CURSOR junctions        (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT DISTINCT j.*
                                                                       FROM JUNCTION j
                                                                                JOIN JUNCTION_POINT jp
                                                                                     ON (j.ID =
                                                                                         jp.JUNCTION_ID)
                                                                                 ...
                                                                                 WHERE rp.ROADWAY_NUMBER IN
                                                                                       (
                                                                                           ...
                                                                                           WHERE rw.VALID_TO IS NULL
                                                                                             ...
                                                                                             AND rw.ROAD_PART_NUMBER =
                                                                                                 ROAD_PART));
    CURSOR junction_points  (ROAD IN INTEGER, ROAD_PART IN INTEGER) IS SELECT jp.*
                                                                                 ...
                                                                                 WHERE rp.ROADWAY_NUMBER IN
                                                                                       (
                                                                                           ...
                                                                                           WHERE rw.VALID_TO IS NULL
                                                                                             ...
                                                                                             AND rw.ROAD_PART_NUMBER =
                                                                                                 ROAD_PART));
    CURSOR road_names       (ROAD IN INTEGER) IS SELECT r_name.*
                                                 FROM ROAD_NAME r_name
                                                 WHERE r_name.VALID_TO IS NULL
                                                   AND r_name.END_DATE IS NULL
                                                   AND r_name.ROAD_NUMBER =
                                                       ROAD;
BEGIN
        ...
        LOOP
            ...
            WHILE RP_INDEX IS NOT NULL
                LOOP
                    ...
                    INSERT INTO EXPORT_TABLE
                    VALUES (COUNTER,
                            '-- Export road address information for:	ROAD_NUMBER = ' ||
                            ROAD ||
                            ' AND ROAD_PART_NUMBER = ' ||
                            ROAD_PART);
                    COUNTER :=
                                COUNTER +
                                1;
                    ...
                    FOR link IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    link."SOURCE" ||
                                    ',' ||
                                    link.ADJUSTED_TIMESTAMP ||
                                    ',' ||
                                    'TO_TIMESTAMP( ''' ||
                                    CAST(
                                            link.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '));');
                    ...
                    FOR roadway IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    roadway.ROAD_NUMBER ||
                                    ',' ||
                                    roadway.ROAD_PART_NUMBER ||
                                    ',' ||
                                    roadway.TRACK ||
                                    ',' ||
                                    roadway.START_ADDR_M ||
                                    ',' ||
                                    roadway.END_ADDR_M ||
                                    ',' ||
                                    roadway.REVERSED ||
                                    ...
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            roadway.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '), ' ||
...
--		  		elem_info_array := SUBSTR(elem_info_array, 1, LENGTH(elem_info_array) - 1) || ')';
                            ...
                            FOR o IN (SELECT TO_CHAR(
                                                     column_value) value
                                      FROM TABLE (
                                          SELECT ll.GEOMETRY.SDO_ORDINATES
                                          FROM LINEAR_LOCATION ll
                                          WHERE ll.ID =
                                                loc.ID))
                                LOOP
...
--					elem_info_array || ',' || ordinate_array || '),'''
                                    ...
                                    'TO_TIMESTAMP( ''' ||
                                    CAST(
                                            loc.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '));');
                    ...
                    FOR rw_point IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            rw_point.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '), '''
                                        ||
                                    rw_point.MODIFIED_BY ||
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            rw_point.MODIFIED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '));');
                    ...
                    FOR cal_point IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    cal_point."TYPE" ||
                                    ',''' ||
                                    cal_point.VALID_FROM ||
                                    ''',''' ||
                                    cal_point.CREATED_BY ||
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            cal_point.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '));');
                    ...
                    FOR node IN
                        ...
                        LOOP
                            ...
                            FOR o IN (SELECT TO_CHAR(
                                                     column_value) value
                                      FROM TABLE (
                                          SELECT n.COORDINATES.SDO_ORDINATES
                                          FROM NODE n
                                          WHERE n.ID =
                                                node.ID))
                                LOOP
...
--					'ST_GeomFromText(''' || 'LINESTRING(' || ordinate_array || ''', 3067), ' || ' '''
                                    ...
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            node.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '), '''
                    ...
                    FOR np IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            np.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '), '
                    ...
                    FOR j IN
                                ...
                                ROAD_PART)
                        LOOP
                            IF j.JUNCTION_NUMBER IS NOT NULL AND
                               j.NODE_NUMBER IS NOT NULL THEN
                                junction_number :=
                                        j.JUNCTION_NUMBER;
                            ELSE
                                junction_number :=
                                        0; -- templates will always be set as junction_number = 0 instead of NULL
                            END IF;
                    ...
                    FOR jp IN
                        ...
                        LOOP
                            ...
                            VALUES (COUNTER,
                                    ...
                                    ''', TO_TIMESTAMP( ''' ||
                                    CAST(
                                            jp.CREATED_TIME AS TIMESTAMP(0)) ||
                                    ''' ' ||
                                    ', ' ||
                                    '''DD-MON-YY HH12.MI.SS PM''' ||
                                    '));');
            ...
            FOR name IN
                ...
                LOOP
                    ...
                    VALUES (COUNTER,
                            ...
                            ''', TO_TIMESTAMP( ''' ||
                            CAST(
                                    name.CREATED_TIME AS TIMESTAMP(0)) ||
                            ''' ' ||
                            ', ' ||
                            '''DD-MON-YY HH12.MI.SS PM''' ||
                            '));');
