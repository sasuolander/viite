...
BEGIN
    ...
    road_section(40925) := (road_part_by_rn(1));
    ...
    WHILE ROAD IS NOT NULL LOOP
            ...
            WHILE RP_INDEX IS NOT NULL LOOP
                    ...
                    FOR roadway IN roads (ROAD, ROAD_PART) LOOP
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             roadway.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(roadway.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || '), ' || roadway.ROAD_TYPE || ',' || roadway.ELY || ',' || roadway.TERMINATED || ',''' || roadway.VALID_FROM || ''') ON CONFLICT (ID) DO NOTHING;');
                            COUNTER := COUNTER + 1;
                        END LOOP;
                    ...
                    FOR loc IN linear_locations (ROAD, ROAD_PART) LOOP
                            ...
                            ordinate_array := REPLACE(ordinate_array, ',', ' ');
                            ordinate_array_comma_pos := INSTR(ordinate_array,' ',1,4); -- FROM DUAL;
                            tmp_ordinate_array := CONCAT(SUBSTR(ordinate_array, 1, ordinate_array_comma_pos-1), ', ');
                            ordinate_array := CONCAT(tmp_ordinate_array, SUBSTR(ordinate_array, ordinate_array_comma_pos - LENGTH(ordinate_array)));
                    ...
                    FOR rw_point IN rw_points (ROAD, ROAD_PART) LOOP
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             rw_point.ID || ',' || rw_point.ROADWAY_NUMBER || ',' || rw_point.ADDR_M || ',''' || rw_point.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(rw_point.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || '), '''

                                                                 ||  rw_point.MODIFIED_BY  || ''', TO_TIMESTAMP( ''' || CAST(rw_point.MODIFIED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || ')) ON CONFLICT (ID) DO NOTHING;');
                    ...
                    FOR cal_point IN cal_points (ROAD, ROAD_PART) LOOP
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             cal_point."TYPE" || ',''' || cal_point.VALID_FROM || ''',''' || cal_point.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(cal_point.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || ')) ON CONFLICT (ID) DO NOTHING;');
                            COUNTER := COUNTER + 1;
                        END LOOP;
                    ...
                    FOR node IN nodes (ROAD, ROAD_PART) LOOP
                            ...
                            ordinate_array := REPLACE(ordinate_array, ',', ' ');
                            ordinate_array_comma_pos := INSTR(ordinate_array,' ',1,2);
                            ordinate_array := CONCAT(SUBSTR(ordinate_array, 1, ordinate_array_comma_pos-1), ')');
                    ...
                    FOR np IN node_points (ROAD, ROAD_PART) LOOP
                            ...
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             np.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(np.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || '), '
                                                                 || node_number || ',' || np."TYPE" || ') ON CONFLICT (ID) DO NOTHING;');
                            COUNTER := COUNTER + 1;
                        END LOOP;
                    ...
                    FOR j IN junctions (ROAD, ROAD_PART) LOOP
                            ...
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             j.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(j.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || '), '''
                                                                 || j.VALID_FROM || ''',' ||
                                                             node_number || ') ON CONFLICT (ID) DO NOTHING;');
                            COUNTER := COUNTER + 1;
                        END LOOP;
                    ...
                    FOR jp IN junction_points (ROAD, ROAD_PART) LOOP
                            INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                             ...
                                                             jp.ID || ',' || jp.BEFORE_AFTER || ',' || jp.ROADWAY_POINT_ID || ',' || jp.JUNCTION_ID || ',''' || jp.VALID_FROM || ''',''' || jp.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(jp.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || ')) ON CONFLICT (ID) DO NOTHING;');
                            COUNTER := COUNTER + 1;
                        END LOOP;
            ...
            FOR name IN road_names (ROAD) LOOP
                    INSERT INTO EXPORT_TABLE VALUES (COUNTER,
                                                     ...
                                                     name.CREATED_BY || ''', TO_TIMESTAMP( ''' || CAST(name.CREATED_TIME AS TIMESTAMP(0))  || ''' '|| ', ' || '''DD-MON-YY HH12.MI.SS PM''' || ')) ON CONFLICT (ID) DO NOTHING;');
                    COUNTER := COUNTER + 1;
                    COMMIT;
                END LOOP;
